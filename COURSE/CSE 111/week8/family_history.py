...
def print_death_age(people_dict):
    ...
    for _ in people_dict:
        ...
        ages_at_death = data[DEATH_YEAR_INDEX] - data[BIRTH_YEAR_INDEX]
        print(name,ages_at_death)
    # NOTE: STRETCH ASSIGNMENT
        # print(f"Birth: {data[BIRTH_YEAR_INDEX]}\nDeath: {data[DEATH_YEAR_INDEX]}\n")
...
def print_marriages(marriages_dict, people_dict):
    ...
    for _ in marriages_dict:
        data = marriages_dict[_]
        husband = data[HUSBAND_KEY_INDEX]
        wife = data[WIFE_KEY_INDEX]
        wedding_year = data[WEDDING_YEAR_INDEX]
        if husband in people_dict:
            husband_name = people_dict[husband][NAME_INDEX]
            husband_age = wedding_year - people_dict[husband][BIRTH_YEAR_INDEX]
        else:
            husband_age = "N/A"
            husband_name = 'N/A'
        if wife in people_dict:
            wife_age = wedding_year - people_dict[wife][BIRTH_YEAR_INDEX]
            wife_name = people_dict[wife][NAME_INDEX]
        else:
            wife_age = 'N/A'
            wife_name = 'N/A'
        print(husband_name, husband_age,">", wedding_year,"<", wife_name, wife_age)


# NOTE: STRETCH ASSIGNMENT
def count_marriages(marriages_dict, people_dict):
    '''counts and prints the number of marriages that each person had in his or her lifetime'''
    print("Number of Marriages")
    marriage_count_dict = {}
    for _ in marriages_dict:
        husband_count = 0
        wife_count = 0
        husband_code = marriages_dict[_][HUSBAND_KEY_INDEX]
        wife_code = marriages_dict[_][WIFE_KEY_INDEX]
        for husband in marriages_dict:
            if husband_code in people_dict: husband_name = people_dict[husband_code][NAME_INDEX]
            if husband_code == marriages_dict[husband][HUSBAND_KEY_INDEX]:
                husband_count += 1
                marriage_count_dict[husband_name] = husband_count
    
        for wife in marriages_dict:
            if wife_code in people_dict: female_name = people_dict[wife_code][NAME_INDEX]
            if wife_code == marriages_dict[wife][WIFE_KEY_INDEX]:
                wife_count += 1
                marriage_count_dict[female_name] = wife_count
    
    for _ in marriage_count_dict:
        print(_, marriage_count_dict[_])
